...
constexpr uint32_t utf8_length(int32_t utf8_codepoint) {
...
constexpr CTRE_FORCE_INLINE pattern_length_result pattern_length(Pattern) noexcept {
	...
	if constexpr (::std::is_same_v<Iterator, utf8_iterator>) {
		...
		if constexpr (size(captures_list{}) && has_backreference(ctll::list<Pattern>()))
			return pattern_length<typename std::basic_string<char>::iterator>(ctll::list<Pattern>(), return_type{}); //we have captures, need these as context
		else
			return pattern_length<typename std::basic_string<char>::iterator>(ctll::list<Pattern>(), int{});
	}
#else
	if constexpr (size(captures_list{}) && has_backreference(ctll::list<Pattern>()))
		return pattern_length<typename std::basic_string<char>::iterator>(ctll::list<Pattern>(), return_type{}); //we have captures, need these as context
...
static constexpr CTRE_FORCE_INLINE pattern_length_result pattern_length(ctll::list<Patterns...>) noexcept {
	...
	if constexpr (::std::is_same_v<Iterator, utf8_iterator>) {
		...
		if constexpr (size(captures_list{}) && has_backreference(ctll::list<Patterns...>()))
			return pattern_length<typename std::basic_string<char>::iterator>(ctll::list<Patterns...>(), return_type{}); //we have captures, need these as context
		else
			return pattern_length<typename std::basic_string<char>::iterator>(ctll::list<Patterns...>(), int{});
	}
#else
	if constexpr (size(captures_list{}) && has_backreference(ctll::list<Patterns...>()))
		return pattern_length<typename std::basic_string<char>::iterator>(ctll::list<Patterns...>(), return_type{}); //we have captures, need these as context
...
static constexpr CTRE_FORCE_INLINE pattern_length_result pattern_length(CharacterLike, R) noexcept {
	...
	if (set.size())
		return pattern_length_result{ utf8_length(set.begin()->low), utf8_length((set.begin()+(set.size() - 1))->high) };
